...
// GenerateStaticHandler generates a smart static handler for production builds
func GenerateStaticHandler(enableSPARouting bool) error {
	...
	var content strings.Builder
	content.WriteString("//go:build !embed_static\n\n")
	content.WriteString("// WARNING: This file is auto-generated by GoFlux CLI.\n")
	content.WriteString("// DO NOT MODIFY - Changes will be overwritten during builds.\n")
	content.WriteString("// Smart static file serving logic managed by GoFlux framework.\n\n")

	content.WriteString("package static\n\n")
	content.WriteString("import (\n")
	content.WriteString("\t\"io/fs\"\n")
	content.WriteString("\t\"net/http\"\n")
	content.WriteString("\t\"os\"\n")
	content.WriteString("\t\"path/filepath\"\n")
	content.WriteString("\t\"strings\"\n")
	content.WriteString(")\n\n")

	// StaticFS function for development
	content.WriteString("// StaticFS returns the development frontend file system\n")
	content.WriteString("func StaticFS() (fs.FS, error) {\n")
	content.WriteString("\treturn os.DirFS(\"frontend/dist\"), nil\n")
	content.WriteString("}\n\n")

	// Smart StaticHandler function
	...
	content.WriteString("\t\t// Remove leading slash\n")
	content.WriteString("\t\tif strings.HasPrefix(path, \"/\") {\n")
	content.WriteString("\t\t\tpath = path[1:]\n")
	content.WriteString("\t\t}\n\n")

	content.WriteString("\t\t// If path is empty, serve index.html\n")
	content.WriteString("\t\tif path == \"\" {\n")
	content.WriteString("\t\t\tpath = \"index.html\"\n")
	content.WriteString("\t\t}\n\n")

	content.WriteString("\t\t// Build full file path\n")
	content.WriteString("\t\tfullPath := filepath.Join(\"frontend/dist\", path)\n\n")

	content.WriteString("\t\t// Check if file exists\n")
	content.WriteString("\t\tif _, err := os.Stat(fullPath); os.IsNotExist(err) {\n")

	if enableSPARouting {
		content.WriteString("\t\t\t// SPA routing: serve index.html for non-file requests\n")
		content.WriteString("\t\t\tif !strings.Contains(path, \".\") {\n")
		content.WriteString("\t\t\t\tfullPath = filepath.Join(\"frontend/dist\", \"index.html\")\n")
		content.WriteString("\t\t\t\tif _, err := os.Stat(fullPath); os.IsNotExist(err) {\n")
		content.WriteString("\t\t\t\t\thttp.NotFound(w, r)\n")
		content.WriteString("\t\t\t\t\treturn\n")
		content.WriteString("\t\t\t\t}\n")
	...
	content.WriteString("\t\t// Set appropriate content type\n")
	content.WriteString("\t\text := filepath.Ext(fullPath)\n")
	content.WriteString("\t\tswitch ext {\n")
	content.WriteString("\t\tcase \".html\":\n")
	content.WriteString("\t\t\tw.Header().Set(\"Content-Type\", \"text/html; charset=utf-8\")\n")
	content.WriteString("\t\tcase \".css\":\n")
	content.WriteString("\t\t\tw.Header().Set(\"Content-Type\", \"text/css; charset=utf-8\")\n")
	content.WriteString("\t\tcase \".js\":\n")
	content.WriteString("\t\t\tw.Header().Set(\"Content-Type\", \"application/javascript; charset=utf-8\")\n")
	content.WriteString("\t\tcase \".json\":\n")
	content.WriteString("\t\t\tw.Header().Set(\"Content-Type\", \"application/json; charset=utf-8\")\n")
	content.WriteString("\t\tcase \".png\":\n")
	content.WriteString("\t\t\tw.Header().Set(\"Content-Type\", \"image/png\")\n")
	content.WriteString("\t\tcase \".jpg\", \".jpeg\":\n")
	content.WriteString("\t\t\tw.Header().Set(\"Content-Type\", \"image/jpeg\")\n")
	content.WriteString("\t\tcase \".svg\":\n")
	content.WriteString("\t\t\tw.Header().Set(\"Content-Type\", \"image/svg+xml\")\n")
	content.WriteString("\t\tcase \".ico\":\n")
	content.WriteString("\t\t\tw.Header().Set(\"Content-Type\", \"image/x-icon\")\n")
	content.WriteString("\t\tcase \".woff\", \".woff2\":\n")
	content.WriteString("\t\t\tw.Header().Set(\"Content-Type\", \"font/woff\")\n")
	content.WriteString("\t\tcase \".ttf\":\n")
	content.WriteString("\t\t\tw.Header().Set(\"Content-Type\", \"font/ttf\")\n")
	content.WriteString("\t\t}\n\n")

	content.WriteString("\t\t// Set caching headers\n")
	content.WriteString("\t\tif strings.Contains(path, \"assets/\") || ext == \".css\" || ext == \".js\" {\n")
	content.WriteString("\t\t\tw.Header().Set(\"Cache-Control\", \"public, max-age=31536000\") // 1 year\n")
	content.WriteString("\t\t} else {\n")
	content.WriteString("\t\t\tw.Header().Set(\"Cache-Control\", \"public, max-age=300\") // 5 minutes\n")
	content.WriteString("\t\t}\n\n")

	content.WriteString("\t\t// Serve the file\n")
	content.WriteString("\t\thttp.ServeFile(w, r, fullPath)\n")
	content.WriteString("\t})\n")
	content.WriteString("}\n")

	handlerFile := filepath.Join(staticDir, "handler.go")
	return os.WriteFile(handlerFile, []byte(content.String()), 0644)
}
